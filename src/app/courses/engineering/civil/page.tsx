...
export default function CivilPage() {
    ...
    const civilProducts = allProducts.filter(p => p.category === "Civil");
                ...
                <div className="container max-w-7xl">
                    <div className="text-center mb-12">
                        ...
                        <p className="mt-2 text-muted-foreground md:text-xl">
                           Explore core subjects of Civil Engineering.
                        </p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {civilProducts.map(product => (
                            <ProductCard key={product.id} product={product} />
                        ))}
                    </div>
                     {civilProducts.length === 0 && (
                        <div className="text-center py-16 text-muted-foreground">
                            <p>No products found for Civil Engineering.</p>
                        </div>
                    )}
                </div>
            </main>
