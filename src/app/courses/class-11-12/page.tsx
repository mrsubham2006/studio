...
const seniorSchoolCategories = [
    "Class 11th", "Class 12th", "JEE", "NEET"
];
...
export default function SeniorSchoolCoursesPage() {
    const allProducts: Product[] = productData.products;
    const seniorSchoolProducts = allProducts.filter(p => seniorSchoolCategories.some(cat => p.category.includes(cat)));
                ...
                <div className="container max-w-7xl">
                    <div className="text-center mb-12">
                        ...
                        <p className="mt-2 text-muted-foreground md:text-xl">
                           Find courses for senior secondary and competitive exams.
                        </p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {seniorSchoolProducts.map(product => (
                            <ProductCard key={product.id} product={product} />
                        ))}
                    </div>
                     {seniorSchoolProducts.length === 0 && (
